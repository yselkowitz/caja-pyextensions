...
CAJA_PYTHON_DIR = os.path.join(os.path.expanduser('~'), '.caja')
PYEXTENSIONS_DIR = os.path.join(CAJA_PYTHON_DIR, 'python-extensions')
PYEXTENSIONS_NOT_ACTIVE_DIR = os.path.join(PYEXTENSIONS_DIR, 'not_active')
BIN_PATH_1 = "/usr/bin/"
BIN_PATH_2 = "/usr/local/bin/"
INSTALL_STRING = "mate-terminal -x gksu apt-get\ -y\ install\ %s"
CONFIG_DIR = os.path.join(os.path.expanduser('~'), '.config/caja-pyextensions')

AVAILABLE_LANGS = ['default', 'en', 'fr', 'it', 'ja']

SHOW_MENU_ICONS = "mateconftool-2 --set /desktop/mate/interface/menus_have_icons --type bool 1"
