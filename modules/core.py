...
class InfoModel:
    ...
    def get_icon_path(self, icon_name):
        ...
        for starting_dir in [cons.GLADE_PATH,
                             '/usr/share/icons/mate', '/usr/share/icons/hicolor', '/usr/share/pixmaps',
                             '/usr/local/share/icons/mate', '/usr/local/share/icons/hicolor', '/usr/local/share/pixmaps']:
...
class CajaPyExtensions:
    ...
    def quit_application(self, *args):
        ...
        if actual_win_size != self.win_size_n_pos['win_size']:
            ...
            self.mateconf_client.set_int("/apps/caja-pyextensions/win_size_w", self.win_size_n_pos['win_size'][0])
            self.mateconf_client.set_int("/apps/caja-pyextensions/win_size_h", self.win_size_n_pos['win_size'][1])
        if actual_win_pos != self.win_size_n_pos['win_position']:
            self.win_size_n_pos['win_position'] = actual_win_pos
            self.mateconf_client.set_int("/apps/caja-pyextensions/win_position_x", self.win_size_n_pos['win_position'][0])
            self.mateconf_client.set_int("/apps/caja-pyextensions/win_position_y", self.win_size_n_pos['win_position'][1])
    ...
    def add_pyextension(self, *args):
        ...
        filepath = self.dialog_open_file(filter_pattern='*.py',
                                         ...
                                         set_dir=self.mateconf_client.get_string("/apps/caja-pyextensions/picking_dir"))
        if filepath != None:
            self.mateconf_client.set_string("/apps/caja-pyextensions/picking_dir", os.path.dirname(filepath))
    ...
    def on_help_menu_item_activated(self, menuitem, data=None):
        ...
        webbrowser.open("http://www.giuspen.com/caja-pyextensions/")
